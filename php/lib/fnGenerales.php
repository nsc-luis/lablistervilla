<?php
	...
	$proceso = isset($_POST['proceso']) ? ($_POST['proceso']) : $_GET['proceso'];
	switch ($proceso) {
		case "login":
			include_once ("cConectaBD.php");
			$bd = new ConectaBD();
			$bd->setNombreBD("labliste_1");
		...
		case "guardarCliente":
			...
			$cte->setLogoEnResultado($_POST['logoEnResultado']);
			$cte->setLogoEnReporte($_POST['logoEnReporte']);
			$cte->setCveWebCte($_POST['cveWebCte']);
			$cte->setPassWebCte($_POST['passWebCte']);
		...
		case "iniciarSesionCliente":
			...
			$cveWebCte = $_POST['cveWebCte'];
			$passWebCte = $_POST['passWebCte'];
			$cveWebCte = stripslashes($cveWebCte);
			$passWebCte = stripslashes($passWebCte);
			
			// consultas a base de datos
			$sql = "SELECT * FROM clientes WHERE cveWebCte = '$cveWebCte' AND passWebCte LIKE BINARY '$passWebCte'";
			$consulta = $bd->consulta($sql);
			while ($row = mysqli_fetch_assoc($consulta)) {
				$idCliente = $row['idCliente'];
				$cveWebCte = $row['cveWebCte'];
			...
			if($count > 0){
				...
				$_SESSION['cveWebCte'] = $cveWebCte;
				$_SESSION['nombreCte'] = $nombreCte;
				$_SESSION['bd'] = "labliste_1";
				$_SESSION['idClinica'] = "1";
				setcookie("idCliente", $idCliente);
				setcookie("cveWebCte", $cveWebCte);
				setcookie("nombreCte", $nombreCte);
				setcookie("bd", "labliste_1");
				setcookie("idClinica", "1");
				$url = "http://" . $_SERVER['SERVER_NAME'] . "/mainClientes.php";
				echo "
					<SCRIPT>
						$( '#msjCliente' ).css('background-color','#A9F5D0');
						$( '#msjCliente' ).html('Acceso correcto!');
						window.location='$url';
					</SCRIPT>
				"; 
			} else {
				echo "
					Clave web y/o contraseña incorrecta.
					<script> $( '#msjCliente' ).css('display','block'); </script>
				";
			}
			$bd->cerrarBD();
		break;
		...
		case "cerrarSesionCliente":
			session_destroy();
			$url = "http://" . $_SERVER['SERVER_NAME'] . "/clientes.php";
			echo $url;
			echo "<SCRIPT>window.location='$url'</SCRIPT>";
		break;
		
		case "cambiarPassWebCte":
			include_once ("cClientes.php");
			$cte = new Clientes();
			$cte->bd->setNombreBD($_SESSION['bd']);
			
			$cte->cambiarPassWebCte($_SESSION['idCliente'], $_POST['nvoPassWebCte']);
			echo "<script> location.reload(); </script>";
			$cte->bd->cerrarBD();
		break;
		
		case "historialPacientePorFechaPortalCliente":
			include_once ("cClientes.php");
			$cte = new Clientes();
			$cte->bd->setNombreBD($_SESSION['bd']);
			
			$historialPacientePorFechaPortalCliente = $cte->historialPacientePorFechaPortalCliente($_SESSION['idCliente'],$_GET['fechaInicial'],$_GET['fechaFinal']);
	
			$data = array();
			if (mysqli_num_rows($historialPacientePorFechaPortalCliente) !=0) {
				while ($row = mysqli_fetch_assoc($historialPacientePorFechaPortalCliente)) {
					$data[] = $row;
				}
			}
			echo $json_info = json_encode($data);
		...
		case "historialPacientePorNoIMSSPortalCliente":
			include_once ("cClientes.php");
			$cte = new Clientes();
			$cte->bd->setNombreBD($_SESSION['bd']);
			
			$historialPacientePorNoIMSSPortalCliente = $cte->historialPacientePorNoIMSSPortalCliente($_SESSION['idCliente'],$_GET['noIMSS']);
			// $historialPacientePorNoIMSSPortalCliente = $cte->historialPacientePorNoIMSSPortalCliente("2",$_GET['noIMSS']);
	
			$data = array();
			if (mysqli_num_rows($historialPacientePorNoIMSSPortalCliente) !=0) {
				while ($row = mysqli_fetch_assoc($historialPacientePorNoIMSSPortalCliente)) {
					$data[] = $row;
				}
			}
			echo $json_info = json_encode($data);
		...
		case "historialPorNombrePacientePortalCliente":
			include_once ("cClientes.php");
			$cte = new Clientes();
			$cte->bd->setNombreBD($_SESSION['bd']);
			
			$historialPorNombrePacientePortalCliente = $cte->historialPorNombrePacientePortalCliente($_SESSION['idCliente'],$_GET['nombreDelPaciente']);
	
			$data = array();
			if (mysqli_num_rows($historialPorNombrePacientePortalCliente) !=0) {
				while ($row = mysqli_fetch_assoc($historialPorNombrePacientePortalCliente)) {
					$data[] = $row;
				}
			}
			echo $json_info = json_encode($data);
		...
		case "getInfoCte":
			...
			$getInfoCte = $cte->getInfoCte($_SESSION['idCliente']);
	
			$data = array();
			if (mysqli_num_rows($getInfoCte) !=0) {
				while ($row = mysqli_fetch_assoc($getInfoCte)) {
					$data[] = $row;
				}
			}
			echo $json_info = json_encode($data);
		...
		case "infoPaciente":
			...
			$infoPaciente = $pte->infoPaciente($_SESSION['idPaciente']);
		...
		case "historialDeEstudiosDelPaciente":
			...
			$idPaciente = isset($_SESSION['idPaciente']) ? $_SESSION['idPaciente'] : $_GET['idPaciente'];
		...
		case "nvaOrdenDeEstudio":
			...
			$url = "http://" . $_SERVER['SERVER_NAME'] . "/main.php?mod=ordenDeEstudio&folio=" . $nvoFolioDocto;
			echo "
				OK! <br />
				<SCRIPT>
					window.location='$url';
