<?php
	...
	class OrdenDeEstudio {
		...
		public function ordenesDeEstudios($str1,$str2){
			...
			$sql = "SELECT o.idDocumento, o.folioDocto, o.idCliente, o.idPaciente, o.importeDocto, 
				o.fechaDocto, o.idUsuario, p.noIMSS, p.nombreCompletoPaciente, c.idCliente, c.nombreCte
				FROM ordendeestudio o
				JOIN clientes c ON c.idCliente = o.idCliente 
				JOIN pacientes p ON p.idPaciente = o.idPaciente 
		...
		public function rptCobPorCte($idUsr, $idCte, $fInicial, $fFinal){
			...
			$sql = "SELECT o.idDocumento, o.folioDocto, o.fechaDocto, o.idPaciente, o.edadDelPaciente, o.claveWebDocto, o.idCliente, o.folioDocto, o.importeDocto, o.observacionDocto, o.publicarResultado, o.idUsuario, c.idCliente, c.nombreCte, p.noIMSS, p.nombreCompletoPaciente
				FROM ordendeestudio o
				JOIN clientes c ON c.idCliente = o.idCliente 
				JOIN pacientes p ON p.idPaciente = o.idPaciente 
				WHERE o.fechaDocto BETWEEN '$fechaInicial' AND '$fechaFinal'
				...
				AND o.idCliente = '$idCte'
				AND o.importeDocto != '0'
				ORDER BY o.folioDocto ASC";
			$data = $this->bd->consulta($sql);
			return $data;
		}
		
		public function rptEstudiosPorDia($idUsr, $fInicial){
			$fechaInicial = $fInicial;
		...
		/* NUEVO */
		public function rptEstDiariosxCte($idCte, $fInicial){
			$fechaInicial = $fInicial;
		...
		public function orden($folio){
			...
			$data = $this->bd->consulta($sql); */
			$sql = "SELECT o.idDocumento, o.folioDocto, o.claveWebDocto, o.fechaDocto, o.observacionDocto, 
				o.idPaciente, o.publicarResultado, p.nombreCompletoPaciente, p.noIMSS, p.genero, 
				p.emailPaciente, o.importeDocto, o.edadDelPaciente, o.idCliente, c.nombreCte 
				FROM ordendeestudio o 
				JOIN clientes c ON c.idCliente = o.idCliente 
				JOIN pacientes p ON p.idPaciente = o.idPaciente 
				WHERE o.folioDocto = '$folio'";
			$data = $this->bd->consulta($sql);
			return $data;
		}
		...
		public function partidasDeLaOrden($folio){
			/* $sql = "SELECT * FROM ods_estudiosypaquetes 
				WHERE folioCte = '$folio' AND idPrecio != '0'
				AND idCliente = '$cte'
				ORDER BY idOds_EyP"; */
			$sql = "SELECT * FROM ods_estudiosypaquetes 
				WHERE folioDocto = '$folio' AND idPrecio != '0'
				ORDER BY idOds_EyP";
			$data = $this->bd->consulta($sql);
			return $data;
		}
		...
		public function agregarEstudioAOrden(){
			...
			$sql = "SELECT * FROM ods_estudiosypaquetes
				WHERE folioDocto = '$this->folioDocto' 
				AND idPrecio = '$this->idPrecio'";
			...
			if (1 == 2) {
				...
				$sql = "INSERT INTO ods_estudiosypaquetes (folioDocto, idPrecio, idCliente, idEstudio, cveEstudio, nombreEstudio, limiteInferiorEstudio, limiteSuperiorEstudio, unidadDeMedida, tipoDeParametro, valorDeReferencia, tipoExamen, precio)
					VALUES ('$this->folioDocto', '$this->idPrecio', '$this->idCliente', '$this->idEstudio', '$this->cveEstudio', '$this->nombreEstudio', '$this->limiteInferiorEstudio', '$this->limiteSuperiorEstudio', '$this->unidadDeMedida', '$this->tipoDeParametro', '$this->valorDeReferencia', '$this->tipoExamen', '$this->precio')";
				...
				$sql = "INSERT INTO ods_estudiosypaquetes (folioDocto, idPrecio, idCliente, idPaquete, cvePaquete, nombrePaquete, tipoExamen, precio)
					VALUES ('$this->folioDocto', '$this->idPrecio', '$this->idCliente', '$this->idPaquete', '$this->cvePaquete', '$this->nombrePaquete', '$this->tipoExamen', '$this->precio')";
				$this->bd->consulta($sql);
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					$sql = "INSERT INTO ods_estudiosypaquetes (folioDocto, idEstudio, cveEstudio, nombreEstudio, limiteInferiorEstudio, limiteSuperiorEstudio, unidadDeMedida, tipoDeParametro, valorDeReferencia, idPaquete, cvePaquete, idCliente)
						VALUES ('$this->folioDocto', '$idEstudio', '$cveEstudio', '$nombreEstudio', '$limiteInferiorEstudio', '$limiteSuperiorEstudio', '$unidadDeMedida', '$tipoDeParametro', '$valorDeReferencia', '$this->idPaquete', '$this->cvePaquete', '$this->idCliente')";
					$this->bd->consulta($sql);
				}
		...
		public function borrarPartidaOrdenDeEstudio($str) {
			...
			if ($this->tipoExamen == 'Paquete') {
				/* $sql = "DELETE FROM ods_estudiosypaquetes
					...
					AND idCliente = '$this->idCliente'"; */
				$sql = "DELETE FROM ods_estudiosypaquetes
					WHERE folioDocto = '$this->folioDocto' AND idPaquete = '$this->idPaquete'";
				$this->bd->consulta($sql);
			}
		...
		public function nvaOrdenDeEstudio() {
			...
			$sql = "SELECT folioDocto FROM ordendeestudio
				ORDER BY folioDocto DESC LIMIT 1";
			$ultimoFolio = mysqli_fetch_assoc($this->bd->consulta($sql));
			//$nvoFolioDocto = $ultimoFolio['folioCte'] + 1;
			$nvoFolioDocto = $ultimoFolio['folioDocto'] + 1;
			...
			$sql = "INSERT INTO ordendeestudio (folioDocto, fechaDocto, idPaciente, edadDelPaciente, claveWebDocto, idCliente, importeDocto, observacionDocto, publicarResultado, idUsuario)
				VALUES ('$nvoFolioDocto', '$fechaActual', '$this->idPaciente', '$edadDelPaciente[edadDelPaciente]', '$claveWebDocto', '$this->idCliente', '0', '', '$pubResult[publicarResultados]', '$this->idUsuario')";
			$this->bd->consulta($sql);
		...
		public function borrarFolioDocto($folio) {
			/* $sql = "DELETE FROM ordendeestudio WHERE folioCte = '$folio'
			AND idCliente = '$cte'"; */
			$sql = "DELETE FROM ordendeestudio WHERE folioDocto = '$folio'";
			$this->bd->consulta($sql);
			/* $sql = "DELETE FROM ods_estudiosypaquetes WHERE folioCte = '$folio'
			AND idCliente = '$cte'"; */
			$sql = "DELETE FROM ods_estudiosypaquetes WHERE folioDocto = '$folio'";
			$this->bd->consulta($sql);
		}
		...
		public function estudiosParaCapturaDeResultados() {
			...
			$sql = "SELECT * FROM ods_estudiosypaquetes
				WHERE folioDocto = '$this->folioDocto' AND cvePaquete = '$this->cvePaquete' AND nombrePaquete = ''
				ORDER BY idOds_EyP ASC";
			$data = $this->bd->consulta($sql);
			return $data;
		}
		...
		public function cambiarCteEnOrden() {
			$sql = "UPDATE ordendeestudio SET idCliente = '$this->idCliente'
				WHERE folioDocto = '$this->folioDocto'";
			$this->bd->consulta($sql);
			$sql = "UPDATE ods_estudiosypaquetes SET idCliente = '$this->idCliente'
				WHERE folioDocto = '$this->folioDocto'";
			$this->bd->consulta($sql);
		}
