<?php
	...
	class OrdenDeEstudio {
		...
		public function ordenesDeEstudios(){
			$fechaFinal = date("Y-m-d");
			$fechaInicial = date("Y-m-d", strtotime("-30 days"));
			
			$filtrarOrdenes = $this->idPerfil == "1" ? "" : "AND idUsuario = '$this->idUsuario'";
			...
			$sql = "SELECT o.idDocumento, o.folioCte, o.idCliente, o.idPaciente, o.importeDocto, o.fechaDocto, o.idUsuario, p.noIMSS, p.nombreCompletoPaciente, c.idCliente, c.nombreCte
		...
		public function orden($cte, $folio){
			$sql = "SELECT o.idDocumento, o.folioCte, o.claveWebDocto, o.fechaDocto, o.observacionDocto, o.idPaciente, o.publicarResultado, p.nombreCompletoPaciente, p.noIMSS, p.genero, p.emailPaciente, o.importeDocto, edadDelPaciente, o.idCliente, c.nombreCte 
				FROM ordendeestudio o 
				JOIN clientes c ON c.idCliente = o.idCliente 
				JOIN pacientes p ON p.idPaciente = o.idPaciente 
		...
		public function nvaOrdenDeEstudio() {
			...
			$sql = "INSERT INTO ordendeestudio (folioCte, fechaDocto, idPaciente, edadDelPaciente, claveWebDocto, idCliente, importeDocto, observacionDocto, publicarResultado, idUsuario)
				VALUES ('$nvoFolioDocto', '$fechaActual', '$this->idPaciente', '$edadActual[0]', '$claveWebDocto', '$this->idCliente', '0', '', '$pubResult[publicarResultados]', '$this->idUsuario')";
			$this->bd->consulta($sql);
		...
		public function buscarResultadoPorClaveWeb($str) {
			...
			$contador = mysqli_num_rows($this->bd->consulta($sql));
			if ($contador == 0) {
				$this->msj = "No se encontro la claveweb solicitada.";
			}
		}
